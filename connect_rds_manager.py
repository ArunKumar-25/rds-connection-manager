...
class RDSConnectionManager:
        ...
        try:
            if BOTO3_AVAILABLE:
                ...
                output = subprocess.check_output(
                    [self.aws_cli, "sts", "get-caller-identity",
    ...
    def generate_auth_token(self, host: str, port: int, region: str,
                           username: str) -> str:
        ...
        try:
            output = subprocess.check_output(
                [self.aws_cli, "rds", "generate-db-auth-token",
                 "--hostname", host,
                 "--port", str(port),
                 "--region", region,
                 "--username", username],
    ...
    def find_databases(self, search_term: Optional[str] = None,
                      exact: bool = False) -> List[DatabaseConfig]:
        ...
        for db in self.catalog:
            ...
            if exact:
                ...
                if (alias.lower() == search_lower or 
                    dbname.lower() == search_lower):
                    matches.append(self._to_config(db))
        
        return matches
    
    def find_suggestions(self, search_term: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Get fuzzy suggestions for partial matches"""
        suggestions = []
        search_lower = search_term.lower()
        
        for db in self.catalog:
            combined = (db.get("dbname", "") + " " + 
                       db.get("alias", "")).lower()
    ...
    def list_databases(self) -> None:
        ...
        print("\nAvailable Databases:")
        print("-" * 80)
        print(f"{'Alias':<20} {'Database':<20} {'Account':<15} {'Env':<8} {'Endpoint':<15}")
        print("-" * 80)
        
        for db in self.catalog:
            env = db.get("env", "").upper()
            prod_marker = " [PROD]" if env == "PROD" else ""
            print(f"{db.get('alias', '-'):<20} "
                  f"{db.get('dbname', '-'):<20} "
    ...
    def connect(self, config: DatabaseConfig, username: str,
...
Examples:
  ...
  %(prog)s mydb --no-prompt
    ...
    parser.add_argument("database", nargs="?",
                       help="Database name or alias to connect to")
    parser.add_argument("--alias", 
                       help="Connect using exact alias match")
    ...
    try:
        ...
        if not search_term:
            try:
                search_term = input("Enter database name or alias (or 'list' to see all): ").strip()
        ...
        if not matches:
            ...
            if suggestions:
                print(f"No exact match for '{search_term}'. Did you mean:", file=sys.stderr)
                for s in suggestions:
                    print(f"  {s['alias']} (db={s['dbname']}, acct={s['account_id']})",
                          file=sys.stderr)
            else:
        ...
        if len(matches) > 1:
            if args.no_prompt:
                print("Multiple matches found with --no-prompt enabled", file=sys.stderr)
                sys.exit(4)
            
            print(f"\nMultiple databases found for '{search_term}':")
            for i, match in enumerate(matches, 1):
                prod = " [PROD]" if match.env.lower() == "prod" else ""
                print(f"  {i}) {match.alias} (db={match.dbname}, "
                      f"account={match.account_id}, env={match.env}){prod}")
            
            try:
                choice = input("\nSelect database (1-{}), or 'q' to cancel: ".format(
                    len(matches))).strip()
            except KeyboardInterrupt:
                print("\nCancelled", file=sys.stderr)
                sys.exit(4)
